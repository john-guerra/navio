...
function NodeNavigator(eleId, x0, y0, h) {
    ...
    canvas = d3.select("#"+eleId).select("canvas")[0][0];
    ...
    function drawItem(item, level) {
        ...
        for (i = 0; i < dDimensions.keys().length; i++) {
            ...
            y = Math.round(yScales[level](item[self.id]) + yScales[level].rangeBand()/2);
            // y = yScales[level](item[self.id]) + yScales[level].rangeBand()/2;

            context.beginPath();
            context.moveTo(x(attrib, level), y);
            context.lineTo(x(attrib, level) + xScale.rangeBand(), y);
            context.lineWidth = Math.round(yScales[level].rangeBand());
            // context.lineWidth = 1;
            ...
            if (yScales[level].rangeBand() > self.divisionsThreshold) {
    ...
    function drawLevelBorder(i) {
        ...
        context.rect(levelScale(i),
            yScales[i].rangeExtent()[0],
            xScale.rangeExtent()[1],
            yScales[i].rangeExtent()[1]-100);
        ...
        tempData = data.reduce(function (p, l, i) {
        ...
        attribHolder.enter()
            ...
            .attr("y", function (d) { return yScales[d.level].rangeExtent()[0]; })
            .attr("width", function () { return xScale.rangeBand(); })
            .attr("height", function (d) { return yScales[d.level].rangeExtent()[1] - yScales[d.level].rangeExtent()[0]; })
    ...
    function drawLevelConnections(level) {
        ...
        data[level].forEach(function (item, i) {
            var locPrevLevel = {x: levelScale(level-1) + xScale.rangeExtent()[1],
            ...
            var points = [ locPrevLevel,
                ...
                locLevel,
                {x: locLevel.x, y: locLevel.y + yScales[level].rangeBand()},
                {x: locLevel.x - self.levelsSeparation * 0.3, y: locLevel.y + yScales[level].rangeBand()},
                {x: locPrevLevel.x + self.levelsSeparation * 0.3, y: locPrevLevel.y + yScales[level - 1].rangeBand()},
                {x: locPrevLevel.x, y: locPrevLevel.y + yScales[level -1 ].rangeBand()},
                locPrevLevel
    ...
    function drawDimensionTitles(level) {
        dDimensions.keys().forEach(function (attrib) {
            ...
            context.save();
            context.translate(x(attrib, level)+xScale.rangeBand()/2 , y0-5);
            context.rotate(-Math.PI/4);
        ...
        context.fillStyle="black";
        context.fillText(data[level].length, levelScale(level), yScales[level].rangeExtent()[1] + 15);
    }
    ...
    function updateScales() {
        ...
        data.forEach(function (levelData, i) {
            yScales[i] = d3.scale.ordinal().rangeBands([y0, h-self.margin - 30], 0.0, 0);
        ...
        xScale
            ...
            .rangeRoundBands([0, self.attribWidth * (dDimensions.keys().length)], 0.1, 0);
        levelScale.domain(data.map(function (d,i) { return i; }))
            .rangeRoundBands([x0+self.margin, ((xScale.rangeExtent()[1] + self.levelsSeparation) * data.length) + x0], 0, 0);
    ...
    self.updateData = function (mData, mColScales, mSortByAttr) {
        ...
        updateScales();
        ctxWidth = levelScale.rangeExtent()[1] + self.margin + x0;
        d3.select(canvas)
            .attr("width", ctxWidth)
            .attr("height", h)
    ...
    self.update = function() {
        var w = levelScale.rangeExtent()[1] + self.margin + x0;
