...
// examples/palettes used to carry a second copy of both, which is how its
// comparison table and the palette it was comparing came to be scored by two
// implementations that could disagree. The example now reads the numbers this
// script writes.
...
/* ------------------------------------------------------------------------- *
 * The comparison table in examples/palettes.
 *
 * Scored by importing what was just written, rather than from the variables
 * above, so the table describes the palettes that actually ship - including
 * mokole and the two ten-colour schemes, which exist in this file only as text
 * inside the template.
 * ------------------------------------------------------------------------- */

const NOTES = {
  nameable:
    "Navio's default. Farthest-point over nameable colours, scored on the worst vision type, opened on the familiar ten.",
  distinct:
    "The same search without the naming floor. Further apart, and some of the colours have no common name.",
  mokole: "Generated by mokole.com/palette.html, a published 50-colour set.",
  category10: "Navio before 0.3.0, and most charts on the web. Ten colours.",
  tableau10: "Tableau's default ten, via d3-scale-chromatic.",
  rainbow:
    "d3.interpolateRainbow sampled at i/n. Cyclical, so it never repeats an endpoint.",
  sinebow: "d3.interpolateSinebow sampled at i/n.",
  turbo:
    "d3.interpolateTurbo. Built for sequential data; included because it is the obvious thing to reach for.",
};

// CIE76: plain Euclidean distance in CIELAB. Kept beside dE00 because they
// disagree, and the disagreement is the point - Heer & Stone (CHI 2012) use
// dE00 and warn that Euclidean CIELAB is unreliable for comparisons across the
// whole space, which is exactly what a minimum-pairwise score is.
const de76 = (x, y) => {
  const a = d3.lab(x),
    b = d3.lab(y);
  return Math.hypot(a.l - b.l, a.a - b.a, a.b - b.b);
};
const minPairWith = (cols, type, metric) => {
  const c = cols.map((x) => simulate(x, type));
  let m = Infinity;
  for (let i = 0; i < c.length; i++)
...
const shipped = await import("../src/palettes.js");
const SIZES = [10, 12, 25, 50];

const sets = {};
for (const n of SIZES) {
  sets[n] = Object.entries(shipped.palettes).map(([name, p]) => {
    const colours = (typeof p === "function" ? p(n) : p.slice(0, n)).map(hex);
    const byType = (metric) =>
      Object.fromEntries(
        TYPES.map((t) => [t, minPairWith(colours, t, metric)])
      );
    const e00 = byType(de00),
      e76 = byType(de76);
    const s = scorePalette(colours);
    return {
      name,
      note: NOTES[name],
      colours,
      // A palette shorter than n does not fail, it RECYCLES - two categories
      // drawn in one colour. Distance cannot express that, so it is its own
      // field and the page says so in words.
      short: colours.length < n ? colours.length : 0,
      // What each colour looks like to a dichromat, precomputed so the page
      // can show it without shipping a colour-appearance model to do it.
      sim: Object.fromEntries(
        TYPES.filter((t) => t !== "normal").map((t) => [
          t,
          colours.map((c) => simulate(c, t)),
        ])
      ),
      de00: e00,
      de76: e76,
      worst00: Math.min(...Object.values(e00)),
      worst76: Math.min(...Object.values(e76)),
      naming: { meanSaliency: +s.meanSaliency, confused: s.confused },
    };
  });
}

writeFileSync(
  new URL("../examples/palettes/palettes.json", import.meta.url),
  JSON.stringify({ sizes: SIZES, sets }, null, 1) + "\n"
);
console.log("wrote examples/palettes/palettes.json");
